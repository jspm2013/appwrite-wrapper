  ...
  try {
    ...
    if (coll) {
      ...
      for (const attr of schema.attributes) {
        ...
        if (!hasRequired) {
          const msg = `Not able to create Attribute '${attr.key}' for Collection '${newArgs.name}' (id: '${newArgs.collectionId}'), because attribute 'required' is missing.`;
          logContent.changes.push({
            action: "createAttribute",
            information: msg,
          });
        ...
        if (attr.key === "relationship") {
          if (newArgs.relationshipAttributeData?.length) {
            newArgs.relationshipAttributeData.map((relAttr: any) => {
              if (
                !relAttr.relationshipKey.length ||
                ...
                throw new Error(
                  `Not able to create Attribute '${newArgs.name}' (id: '${newArgs.collectionId}'), since relationship information for key '${attr.key}' is missing (relationshipKey or relatedCollectionId or both, passed with param 'relationshipAttributeData' are are not valid)`
                );
              }
            });
          } else {
            ...
            throw new Error(
              `Not able to create Attribute '${newArgs.name}' (id: '${newArgs.collectionId}'), since relationship information for key '${attr.key}' is missing (relationshipAttributeData)`
            );
          }
        }
        ...
        try {
          await createAttribute(
            ...
            newArgs.collectionId!,
            attr,
            newArgs.relationshipAttributeData?.find(
              (relAttr) => relAttr.relationshipKey === attr.key
            )?.relatedCollectionId
          );
...
const listDocuments = async (
  ...
  try {
    ...
    const matchesValues = (docVal: any, targets: string[]): boolean => {
      if (Array.isArray(docVal)) {
  ...
  try {
    ...
    for (const schemaAttr of schema.attributes) {
      if (!currentAttributeKeys.has(schemaAttr.key)) {
        ...
        // Keys validation: can't have both xdefault and required
        const hasRequired =
          "required" in schemaAttr && schemaAttr.required !== undefined;
        if (!hasRequired) {
          const msg = `Not able to create Attribute '${schemaAttr.key}' for Collection '${newArgs.name}' (id: '${newArgs.collectionId}'), because attribute 'required' is missing.`;
          logContent.changes.push({
            action: "createAttribute",
            information: msg,
          });
          ...
          if (newArgs.relationshipAttributeData?.length) {
            newArgs.relationshipAttributeData.map((relAttr: any) => {
              if (
                ...
                throw new Error(
                  `Not able to create Attribute '${newArgs.name}' (id: '${newArgs.collectionId}'), since relationship information for key '${schemaAttr.key}' is missing (relationshipKey or relatedCollectionId or both, passed with param 'relationshipAttributeData' are are not valid)`
                );
              }
            });
          } else {
            ...
            throw new Error(
              `Not able to create Attribute '${newArgs.name}' (id: '${newArgs.collectionId}'), since relationship information for key '${schemaAttr.key}' is missing (relationshipAttributeData)`
            );
          }
        }
        ...
        try {
          await createAttribute(
            ...
            schemaAttr,
            newArgs.relationshipAttributeData?.find(
              (relAttr) => relAttr.relationshipKey === schemaAttr.key
            )?.relatedCollectionId
          );
          ...
          throw new Error(
            `Couldn't create attribute '${schemaAttr}': ${error}`
          );
        }
        ...
        if (!attributesEqual(existingAttr!, schemaAttr)) {
          ...
          try {
            ...
            throw new Error(
              `Couldn't update attribute '${schemaAttr}': ${error}`
            );
          }
        } else {
          logContent.changes.push({
    ...
    if (args.destructive) {
      ...
      for (const key of attributesToRemove) {
        ...
        const deletedAttribute = await deleteAttribute({
          databaseId: newArgs.databaseId!,
          collectionId: newArgs.collectionId!,
          key,
        });

        if (deletedAttribute?.error) {
          throw new Error(
            `Couldn't delete attribute '${key}': ${deletedAttribute?.error}`
          );
        }
    ...
    if (args.destructive) {
      ...
      const indexesToDelete = existingIndexes.filter(
        (idx) => !schemaIndexKeys.has(idx.key)
      );
      ...
      for (const index of indexesToDelete) {
        ...
        try {
          ...
          throw new Error(
            `Couldn't delete index '${index.key}': ${error.message}`
          );
        }
      }
    }
