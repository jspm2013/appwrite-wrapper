 ...
 */
export const temporaryPassword = (length = 12): string => {
  const characters =
...
export const imgToWebP = async (
  ...
  width?: number,
  height?: number,
  background: string = "transparent" // Default is transparent
): Promise<File> => {
...
export const humanTimeStamp = (timestamp?: number): string => {
  const date = new Date(timestamp ?? Date.now());
  const humanTs = `${date.getFullYear()}${(date.getMonth() + 1)
    .toString()
